...
if not wait(function() return require("opencode-nvim.sse").connected() end, 15000) then
  io.write("não conectei no stream de eventos\n")
  os.exit(1)
end
...
if not wait(function() return info ~= nil end, 90000) then
  io.write("não consegui criar a sessão\n")
  os.exit(1)
end

report("detecta o agente que pede aprovação", info.agent == AGENT and info.approval == true,
...
plugin.prompt("Crie um arquivo chamado " .. TARGET .. " contendo exatamente: oi")
...
report("o turno terminou", state.finished, "timeout esperando o fim do turno")
report("pediu permissão antes de gravar", #state.asked > 0,
  "nenhuma permissão pedida (o agente ignorou as regras?)")
report("aprovamos pelo popup", state.approvals > 0, "nenhuma aprovação enviada")

local target = vim.fs.joinpath(workdir, TARGET)
local exists = vim.fn.filereadable(target) == 1
report("arquivo criado depois da aprovação", exists, "arquivo não existe: " .. target)
if exists then
  io.write("   conteúdo: " .. vim.inspect(vim.fn.readfile(target)) .. "\n")
end

-- The review popup must stay out of the way when pre-approval is active.
report("não abre o popup de revisão no modo aprovação", session.approval_active(), "approval_active() falso")
...
session.diff({ context = 3 }, function(err, patches, source)
  ...
  if err then io.write("   diff ERRO: " .. vim.inspect(err) .. "\n") end
end)
wait(function() return diff_done end, 20000)
report("o diff do turno tem conteúdo", type(diff_patches) == "table" and #diff_patches > 0,
  vim.inspect(diff_patches))
if type(diff_patches) == "table" and #diff_patches > 0 then
  io.write(string.format("   fonte=%s  %s (+%d -%d) status=%s\n", tostring(diff_source),
...
report("desfazer funcionou", reverted and revert_err == nil, tostring(revert_err))
vim.wait(600, function() return false end, 50)
local still_there = vim.fn.filereadable(target) == 1
report("desfazer removeu o arquivo criado", not still_there,
  "o arquivo continua no disco depois do revert")
